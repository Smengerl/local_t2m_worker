...
| [`health_check.sh`](#health_checksh) | Shell | Check system status at a glance |
...
Cyclic status check that refreshes every ~5 seconds (Ctrl-C to quit). Useful for monitoring during a generation or download.
...
✔ Server reachable (localhost:8000)
✖ Generation running
✔ HF download   10.1 MB/s (via en0)
✖ Web GUI traffic  2 B/s (lo0)
```

| Line | What is checked |
|---|---|
| **Batch worker running** | `batch/worker.pid` exists and the PID responds to `kill -0` |
...
| **Generation running** | At least one job with `status=running` in `queue.jsonl` |
